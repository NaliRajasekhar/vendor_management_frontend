...
export async function searchClients(q = '', { limit = 50, offset = 0 } = {}) {
  const { data } = await axios.get(`${base}/api/search/clients`, { params: { q, limit, offset } })
  return data
}
...
export async function getVendorClientsByVendor(vendor_id, { q = '', limit = 50, offset = 0 } = {}) {
  const { data } = await axios.get(`${base}/api/search/vendor-clients`, { params: { vendor_id, q, limit, offset } })
  return data
}

export async function listClientOptions(params = {}) {
  const { vendor_id } = params || {}
  const { data } = await axios.get(`${base}/api/search/client-options`, { params: { vendor_id } })
  return data?.items || []
}
...
export async function getVendorsByClient(client_name, { q = '', limit = 50, offset = 0 } = {}) {
  const { data } = await axios.get(`${base}/api/search/client-vendors`, { params: { client_name, q, limit, offset } })
  return data
}

export async function searchGlobalVendorClients(q = '', { limit = 50, offset = 0 } = {}) {
  const { data } = await axios.get(`${base}/api/search/global`, { params: { q, limit, offset } })
  return data
}
